...
</li>`}function h(t){const e=document.querySelector(".bestsellers-container");e.innerHTML="",B(".bestsellers-container","Best Sellers Books"),e.insertAdjacentHTML("beforeend",'<ul class="bestsellers-list"></ul>');const r=document.querySelector(".bestsellers-list"),s=t.map(({books:o,list_name:a})=>`
            <li class="bestsellers-item">
             <h2 class="bestsellers-category-title">${a}</h2>
                <ul class="bestsellers-books-list"> 
                    ${o.map(({book_image:n,title:c,author:l})=>$({book_image:n,title:c,author:l})).join(`
`)}
                </ul>
                <button class="bestsellers-btn" type="button data-category="${a}">See more</button>
            </li>`).join(`
  ...
  <div class="book-category-card">
    ...
    <div class="book-category-text">
      ...
      <p class = "author-name"> ${e}</p>
        ...
        <ul class="category-books">
        ${t.map(({_id:o,book_image:a,title:n,author:c})=>`<li class="category-book-card" id='${o}'>
        <div class="category-box-overlay">
            <img class="category-book-image" data-id="${o}" src="${a}" alt="Book">
        ...
        </ul>`;g.insertAdjacentHTML("afterbegin",r),document.querySelectorAll(".category-book-card").forEach(o=>{o.addEventListener("click",x)})}function x(t){const e=t.target.dataset.id;k.putProducts({_id:e}),document.getElementById(e).remove()}document.addEventListener("DOMContentLoaded",function(){const t=document.getElementById("modal"),e=document.querySelector(".modal-close"),r=document.querySelector(".modal-add-to-cart"),s=new f;function o(){t.style.display="none",document.body.style.overflow="auto"}e.addEventListener("click",o),t.addEventListener("click",function(c){c.target===t&&o()}),document.addEventListener("keydown",function(c){c.key==="Escape"&&o()});function a(c){const l=t.querySelector(".book-cover"),b=t.querySelector(".book-title"),S=t.querySelector(".author"),L=t.querySelector(".description"),m=t.querySelector(".links");l.src=c.coverImageUrl,b.textContent=c.title,S.textContent="Author: "+c.author,L.textContent=c.description,m.innerHTML="",c.links.forEach(function(y){const d=document.createElement("a");d.href=y.url,d.textContent=y.name,d.target="_blank",m.appendChild(d)}),s.checkProduct(c.id).hasProd?r.textContent="Remove from Shopping List":r.textContent="Add to Shopping List"}a({id:1,coverImageUrl:"https://example.com/book-cover.jpg",title:"Example Book Title",author:"John Doe",description:"Lorem ipsum dolor sit amet, consectetur adipiscing elit.",links:[{name:"Amazon",url:"https://www.amazon.com"},{name:"Apple Books",url:"https://books.apple.com"}]})});const g=document.querySelector(".home-page");
